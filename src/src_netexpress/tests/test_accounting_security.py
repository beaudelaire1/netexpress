...
from accounting.services import invoice_fingerprint, is_reviewed, issued_invoices
...
def make_user(role="accountant", verified=True):
    user = User.objects.create_user(
        ...
        email=f"u{User.objects.count()}@example.test",
...
def test_public_roles_cannot_escalate(client):
    form = SignUpForm({
        ...
        "password1": "LongSecurePassword854!",
        "password2": "LongSecurePassword854!",
    })
...
def test_invitation_single_use_and_role_redirect(client):
    ...
    url = reverse(
        "accounts:password_setup",
        args=[urlsafe_base64_encode(force_bytes(user.pk)), token],
    )
    response = client.post(url, {
        "new_password1": "AnotherSecure854!",
        "new_password2": "AnotherSecure854!",
    })
...
def test_login_external_next_is_rejected(client):
    ...
    response = client.post(
        ...
        {"username": user.username, "password": "LongSecurePassword854!"},
    )
...
def test_numbers_after_999_and_soft_delete(kind):
    ...
    fields = {
        ...
        **({"client": customer} if kind == "quote" else {}),
    }
...
def test_accounting_hides_unfinished_client_invoices(client):
    invoice = make_invoice()
    user = make_user()
    client.force_login(user)
    detail_url = reverse("accounting:invoice_detail", args=[invoice.pk])
    ...
    assert not issued_invoices().filter(pk=invoice.pk).exists()
    ...
    assert client.get(detail_url).status_code == 404
    assert client.get(reverse("accounting:sales")).context["page"].paginator.count == 0
...
def test_supplier_upload_total_duplicate_and_private_download(client):
    ...
    client.force_login(user)
    response = client.post(
        reverse("accounting:supplier_add"),
        {**purchase_data(), "file": upload()},
    )
...
@pytest.mark.parametrize(
    "file,total,vat",
    [
        (upload(b"<html>bad</html>"), "120", "20"),
        (upload(filename="piece.html"), "120", "20"),
        (upload(), "-1", "0"),
        (upload(), "120", "121"),
    ],
)
def test_supplier_rejects_invalid_files_and_amounts(file, total, vat):
    form = SupplierInvoiceForm(
        purchase_data(total_ttc=total, vat_amount=vat),
        {"file": file},
    )
...
def test_supplier_control_edit_resets_review(client):
    ...
    client.post(
        reverse("accounting:review_supplier", args=[purchase.pk]),
        {"fingerprint": purchase.updated_at.isoformat()},
    )
    purchase.refresh_from_db()
    assert purchase.reviewed_at
    client.force_login(owner)
    response = client.post(
        reverse("accounting:supplier_edit", args=[purchase.pk]),
        {**purchase_data(total_ttc="130"), "version": purchase.updated_at.isoformat()},
    )
    assert response.status_code == 302
    purchase.refresh_from_db()
...
def test_exports_period_formulas_zip_and_totals(client):
    ...
    client.post(
        reverse("accounting:supplier_add"),
        {**purchase_data(supplier_name="=CMD()"), "file": upload()},
    )
    client.force_login(make_user())
    selection = {"date_from": "2026-01-01", "date_to": "2026-12-31"}
    response = client.get(reverse("accounting:export"), selection)
    assert response.status_code == 200
    journal = response.content.decode("utf-8-sig")
    assert "'=CMD()" in journal and "120.00" in journal
    ...
    with patch.object(Invoice, "generate_pdf", return_value=b"%PDF-1.4 test"):
        ...
        with zipfile.ZipFile(io.BytesIO(b"".join(response.streaming_content))) as archive:
            ...
            assert not any(name.startswith("devis/") for name in names)

    response = client.get(reverse("accounting:dashboard"), selection)
    totals = response.context["totals"]
    assert totals["purchases"] == Decimal(120)
    assert totals["purchases_ht"] == Decimal(100)
    assert totals["purchase_vat"] == Decimal(20)
    assert totals["net_sales"] == Decimal(60)
    assert totals["net_sales_ht"] == Decimal(50)
    assert totals["sales_vat"] == Decimal(10)
    assert response.context["financial_series"]
    assert client.get(
        reverse("accounting:export"),
        {"date_from": "2027-12-31", "date_to": "2026-01-01"},
    ).status_code == 400


def test_accountant_invite_disable(client):
    client.force_login(make_user("admin_business"))
    response = client.post(reverse("accounting:accountants"), {
        "email": "cabinet@example.test",
        "first_name": "Anne",
        "last_name": "Test",
        "accounting_firm": "Cabinet Test",
    })
    assert response.status_code == 302
...
def test_admin_dashboard_has_accounting_navigation_and_modern_kpis(client):
    client.force_login(make_user("admin_business"))
    ...
    assert 'aria-label="Indicateurs comptables de la période"' in portal
    assert portal.count("data-accounting-chart") == 2
    assert "Ventes HT" in portal and "TVA collectée" in portal
    assert "Achats HT" in portal and "TVA sur achats" in portal
    assert 'aria-label="Préparer des pièces"' in portal
...
def test_only_company_can_deposit(client, role, allowed):
    ...
    for route in ("supplier_add", "document_add"):
        ...
        response = client.post(
            url,
            {"file": upload(f"%PDF-1.4 {role} {route}".encode())},
        )
        assert response.status_code == (302 if allowed else 403), response.content[:1000]


def test_supplier_draft_is_private_until_complete(client):
    admin = make_user("admin_business")
    ...
    purchase = SupplierInvoice.objects.get()
    assert not purchase.is_complete

    admin_dashboard = client.get(reverse("accounting:dashboard"))
    assert admin_dashboard.context["totals"]["incomplete_purchases"] == 1
    assert client.get(reverse("accounting:supplier_detail", args=[purchase.pk])).status_code == 200
    assert client.get(purchase.file.url).status_code == 200

    accountant = make_user()
    client.force_login(accountant)
    dashboard = client.get(reverse("accounting:dashboard"))
    assert dashboard.context["totals"]["incomplete_purchases"] == 0
    assert dashboard.context["totals"]["purchases"] == 0
    ...
    export = client.get(reverse("accounting:export"))
    assert purchase.display_name not in export.content.decode("utf-8-sig")
...
def test_supplier_optional_metadata_validation_and_completion(client):
    ...
    duplicate = SupplierInvoiceForm(
        purchase_data(supplier_name=" FOURNISSEUR "),
        {"file": upload(b"%PDF-1.4 different")},
    )
...
def test_company_document_then_cabinet_review_and_download(client, kind):
    ...
    client.force_login(owner)
    response = client.post(
        reverse("accounting:document_add"),
        {"file": upload(filename="releve-aout.pdf"), "kind": kind},
    )
    assert response.status_code == 302, response.content[:1500]
    ...
    assert client.post(
        url,
        {"fingerprint": document.updated_at.isoformat(), "note": "Pièce lisible"},
    ).status_code == 302
...
def test_document_security_duplicates_and_stale_changes(client):
    ...
    document.refresh_from_db()
    response = client.post(
        reverse("accounting:document_edit", args=[document.pk]),
        {"title": "Old edit", "version": original_version},
    )
    ...
    client.post(
        reverse("accounting:document_edit", args=[document.pk]),
        {"title": "New title", "version": document.updated_at.isoformat()},
    )
    document.refresh_from_db()
...
def test_company_cannot_perform_cabinet_controls(client):
    ...
    document = AccountingDocument.objects.get()
    for route, piece in [
        ("review_invoice", invoice),
        ("review_supplier", purchase),
    ...
    assert "Marquer comme vérifié" not in client.get(
        reverse("accounting:document_detail", args=[document.pk])
    ).content.decode()
...
@pytest.mark.parametrize(
    "content,filename",
    [
        (b"<html>bad</html>", "bad.pdf"),
        (b"%PDF-1.4", "bad.html"),
        (b"", "empty.pdf"),
        (b"%PDF-" + b"x" * (10 * 1024 * 1024), "large.pdf"),
    ],
    ids=["invalid-signature", "invalid-extension", "empty", "too-large"],
)
...
def test_document_filters_exports_and_no_effect_on_sales(client):
    ...
    client.post(
        reverse("accounting:document_add"),
        {"file": upload(), "kind": "bank", "title": "=SUM(1)", "document_date": "2026-08-01"},
    )
    client.post(
        reverse("accounting:document_add"),
        {"file": upload(b"%PDF-1.4 old"), "kind": "contract", "document_date": "2025-08-01"},
    )
    client.force_login(make_user())
    selection = {"date_from": "2026-01-01", "date_to": "2026-12-31"}
    ...
    assert totals["pending_documents"] == 1
    assert totals["net_sales"] == totals["purchases"] == 0
    response = client.get(reverse("accounting:export"), {**selection, "format": "zip"})
    with zipfile.ZipFile(io.BytesIO(b"".join(response.streaming_content))) as archive:
...
def test_quote_is_only_context_for_visible_invoice(client):
    invoice = make_invoice()
    quote = invoice.quote
    unrelated = Quote.objects.create(client=quote.client, status="sent")
    QuoteItem.objects.create(
        quote=unrelated,
        description="Autre proposition",
        quantity=1,
        unit_price=50,
        tax_rate=0,
    )
    unrelated.compute_totals()

    client.force_login(make_user())
    dashboard_html = client.get(reverse("accounting:dashboard")).content.decode()
    assert 'href="/comptabilite/devis/"' not in dashboard_html
    invoice_html = client.get(reverse("accounting:invoice_detail", args=[invoice.pk])).content.decode()
    assert reverse("accounting:quote_detail", args=[quote.pk]) in invoice_html

    detail = client.get(reverse("accounting:quote_detail", args=[quote.pk]))
    assert detail.status_code == 200
    assert f"Facture {invoice.number}" in detail.content.decode()
    assert client.get(reverse("accounting:quote_detail", args=[unrelated.pk])).status_code == 404
    assert client.get(reverse("accounting:quote_pdf", args=[unrelated.pk])).status_code == 404

    before = (quote.total_ttc, quote.status, quote.pdf.name)
    with patch(
        "core.services.document_generator.DocumentGenerator.generate_quote_pdf",
        return_value=b"%PDF-1.4 quote",
    ) as generate:
    ...
    with patch.object(Invoice, "generate_pdf", return_value=b"%PDF-1.4 invoice"):
        response = client.get(reverse("accounting:export"), {"format": "zip"})
        with zipfile.ZipFile(io.BytesIO(b"".join(response.streaming_content))) as archive:
            assert "devis.csv" not in archive.namelist()
            assert not any(name.startswith("devis/") for name in archive.namelist())
...
def test_turnstile_failure_denies(settings):
    ...
    with patch("core.turnstile.requests.post", side_effect=OSError):
        assert not verify_turnstile(
            RequestFactory().post("/", {"cf-turnstile-response": "test"})
        )
