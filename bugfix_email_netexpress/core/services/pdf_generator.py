...
def render_invoice_pdf(invoice: Any, *, extra_context: Optional[Dict[str, Any]] = None) -> PdfFile:
    """Render an invoice to premium PDF bytes using WeasyPrint."""
    # Reuse the canonical service to keep a single rendering implementation.
    from core.services.pdf_service import InvoicePdfService  # local import to avoid cycles

    service = InvoicePdfService()
    pdf_file = service.generate(invoice)
    # Allow callers to override filename if needed (rare)
    if extra_context and extra_context.get("filename"):
        return PdfFile(filename=str(extra_context["filename"]), content=pdf_file.content)
    return PdfFile(filename=pdf_file.filename, content=pdf_file.content)


def generate_invoice_pdf(invoice: Any, context: Optional[Dict[str, Any]] = None) -> str:
    """Backward-compatible helper: write invoice PDF to MEDIA_ROOT and return relative path."""
    pdf_res = render_invoice_pdf(invoice, extra_context=context or {})
    out_dir = Path(settings.MEDIA_ROOT) / "factures" / "pdf"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / pdf_res.filename
    out_path.write_bytes(pdf_res.content)
    return f"factures/pdf/{pdf_res.filename}"
