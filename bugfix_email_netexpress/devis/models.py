...
# Import the unified Customer model from the crm app.  This replaces
# the legacy Client model defined in previous versions of the project.
...
# Deprecation notice
# ---------------------------------------------------------------------------
# The original ``Client`` model has been superseded by the unified
# ``Customer`` model defined in ``crm.models``.  To preserve backwards
# compatibility with existing imports (e.g. ``from devis.models import Client``),
# ``Client`` is defined as an alias pointing to ``crm.models.Customer``.
Client = Customer
...
class Quote(models.Model):
    ...
    # Message email personnalisable (HTML) pour l'envoi du devis.
    # Rempli via l'éditeur WYSIWYG (TinyMCE) dans le back-office.
    email_body_html = models.TextField(blank=True, default="", help_text="Message HTML inséré dans l'email d'envoi du devis (optionnel).")
