...
def compute_quote_totals(quote: Quote, *, persist: bool = False) -> None:
    """Calcule les totaux HT/TVA/TTC d'un devis à partir de ses lignes.

    Cette fonction remplace l'ancienne "hexagonale morte" afin de garder une
    Service Layer simple, testable et 100% Django.
    """
    ...
    quote.total_ht = total_ht.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    quote.tva = total_tva.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    quote.total_ttc = (quote.total_ht + quote.tva).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    if persist:
        quote.save(update_fields=["total_ht", "tva", "total_ttc"])
