...
# Cette section utilise django‑environ pour lire automatiquement les
# variables définies dans un fichier .env à la racine du projet.  Cela
# permet de configurer facilement des paramètres sensibles (comme les
# identifiants SMTP) sans les écrire en clair dans le code.  Les valeurs
# chargées sont injectées dans os.environ, ce qui rend les appels
# `os.getenv(...)` ci‑dessous fonctionnels.
try:
    import environ  # type: ignore

    env = environ.Env()
    # Rechercher un fichier .env à la racine du projet et le charger
    env.read_env(os.path.join(Path(__file__).resolve().parent.parent.parent, ".env"))
except Exception:
    # Si django‑environ n'est pas installé ou qu'aucun fichier .env n'est
    # disponible, continuer sans charger explicitement de variables.
    pass
...
# ---------------------------------------------------------------------------
# Le projet envoie des notifications par e‑mail (factures, devis, messages de
# contact, rappels de tâches).  Pour fonctionner correctement, Django doit
# connaître les paramètres SMTP.  Plutôt que d’inscrire les identifiants
# directement dans le code, ces variables lisent les valeurs depuis
# l’environnement (fichier .env ou variables système).  Les valeurs
# fournies ici servent de défauts raisonnables pour un compte Gmail et
# peuvent être surchargées par vos propres paramètres.
...
# Destinataires par défaut pour les notifications
